...
To use the GitHub JWT authentication type, you need to set up a GitHub App that allows for your connection to come from a personal account (much like a personal access token) or from an organization account you own or have access to. One of the major benefits of using a GitHub App is:
...
When asking for permissions, the application will need at least read access to "Contents" to be able to list and pull files in from the repo.

Using the information above, navigate to Operations > Data Source and click Add. Select the type to be "GitHub (JWT)". Fill out the JWT Private Key, App ID, and in the Access Token API URL use one of the following URLs that will include your Installation ID:
